...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (ribbon) {
    ...
    if (progressPath && dot && typeof progressPath.getTotalLength === 'function') {
      ...
      const enableRibbonGestureMotion = () => {
        const canReactToPointer = window.matchMedia && window.matchMedia('(hover: hover) and (pointer: fine) and (min-width: 901px) and (prefers-reduced-motion: no-preference)').matches;
        const ribbonSvg = ribbon.querySelector('.vpl-scroll-ribbon-svg');
        if (!canReactToPointer || !ribbonSvg) return;
        ...
        const setRibbonMotion = (x, y, rotate) => {
          ribbon.style.setProperty('--vpl-ribbon-motion-x', `${x.toFixed(2)}px`);
          ribbon.style.setProperty('--vpl-ribbon-motion-y', `${y.toFixed(2)}px`);
          ribbon.style.setProperty('--vpl-ribbon-motion-rotate', `${rotate.toFixed(3)}deg`);
        };

        const tick = (time) => {
          const idle = time - lastGesture > 80;

          // On ne colle plus le ruban à la position de la souris :
          // il garde uniquement une inertie basée sur la direction du geste,
          // puis revient naturellement sur sa trajectoire d'origine.
          if (idle) {
            targetX *= .88;
            targetY *= .88;
            targetRotate *= .84;
          ...
          currentX += (targetX - currentX) * .17;
          currentY += (targetY - currentY) * .17;
          currentRotate += (targetRotate - currentRotate) * .16;
          setRibbonMotion(currentX, currentY, currentRotate);

          const energy = Math.abs(targetX) + Math.abs(targetY) + Math.abs(currentX) + Math.abs(currentY) + Math.abs(targetRotate) * 8 + Math.abs(currentRotate) * 8;
          if (energy > .18) {
            ...
            currentY = 0;
            currentRotate = 0;
            setRibbonMotion(0, 0, 0);
            ribbon.classList.remove('is-gesture-active');
          }
        };
        ...
        const onPointerMove = (event) => {
          ...
          const dt = clamp(now - lastTime, 12, 80);
          ...
          const velocityX = dx * (16 / dt);
          const velocityY = dy * (16 / dt);
          const speed = Math.hypot(velocityX, velocityY);
          if (speed < .55) return;

          const width = window.innerWidth || 1;
          const height = window.innerHeight || 1;
          const maxX = Math.min(96, Math.max(34, width * .062));
          const maxY = Math.min(78, Math.max(28, height * .075));

          targetX = clamp(targetX + velocityX * .42, -maxX, maxX);
          targetY = clamp(targetY + velocityY * .48, -maxY, maxY);
          targetRotate = clamp(targetRotate + (velocityX * .018) - (velocityY * .012), -4.5, 4.5);
          lastGesture = now;
          ribbon.classList.add('is-gesture-active');
        ...
        document.addEventListener('pointerleave', resetGesture, { passive: true });
        window.addEventListener('blur', resetGesture);
      };

      enableRibbonGestureMotion();
